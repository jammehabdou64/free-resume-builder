...
# ORM: jcc (Knex), sequelize, or mongoose
DB_ORM=jcc

# MongoDB (when DB_ORM=mongoose)
MONGODB_URI=mongodb://localhost:27017/jcc_express

# Generated file extension: ts or js (for make:model, make:controller, etc.)
FILE_EXT=ts
...
// Jobs with delay are automatically handled (delay is in seconds)
const job = new ProcessPodcast(podcast);
job.delay = 5; // 5 seconds
await dispatch(job);
```
...
import { DatabaseServiceProvider } from "jcc-express-mvc/lib/Database/DatabaseServiceProvider";

export const providers = [
  AppServiceProvider,
  EventServiceProvider,
  QueueServiceProvider,
  DatabaseServiceProvider, // Database connection & model loading (auto-registered by ApplicationBuilder)
  // Add your custom providers here
];
```

**Note:** `DatabaseServiceProvider` is automatically registered first by `ApplicationBuilder.withProviders()` when using the default setup. It manages the Database module (Knex, Sequelize, Mongoose drivers) and loads Sequelize models when `DB_ORM=sequelize`.
...
JCC Express MVC provides a standalone CSRF (Cross-Site Request Forgery) protection middleware. CSRF protection is opt-in — add it to your Kernel when you need it.

### Installation

Import `csrf` from the framework and add it to your Kernel's global middleware array. **It must be registered after `express-session` and `cookieParser`**, since it relies on sessions to store the token.
...
- On every request, the middleware ensures a CSRF token exists in the session.
- The token is exposed via `res.locals._token` (available in templates) and `req.csrfToken()` (programmatic access).
- On **unsafe** HTTP methods (`POST`, `PUT`, `PATCH`, `DELETE`), the middleware validates the submitted token against the session token.
- The token is checked in the `_token` form field and the `x-csrf-token` HTTP header.
- If validation fails, a **419 Page Expired** response is returned.
- API routes (`/api/*`) are skipped by default.

### Configuration Options

```typescript
csrf({
  except: ["/webhooks", /^\/stripe/], // Routes to exclude from CSRF checks
  sessionKey: "_csrf_token", // Session key for storing the token
  fieldName: "_token", // Form field name to check
  headerName: "x-csrf-token", // HTTP header name to check
  tokenLength: 32, // Token byte length
  skipApi: true, // Skip /api/* routes (default: true)
});
```

### Using in Templates

Use the `@csrf` directive inside any `<form>`:
...
```html
@js const csrfToken = '{{ _token }}'; @endjs
...
Or include a `<meta>` tag in your layout:
...
If you have routes that should not be CSRF-protected (e.g. webhooks), add them to the `except` option:

```typescript
csrf({
  except: ["/webhooks/stripe", "/webhooks/github", /^\/external-api/],
});
```

## Method Spoofing

HTML forms only support `GET` and `POST` methods. JCC Express MVC provides a method spoofing middleware that allows forms to submit `PUT`, `PATCH`, and `DELETE` requests.

### Installation

Import `methodSpoofing` from the framework and add it to your Kernel:
...
- On `POST` requests, the middleware checks for a `_method` field in the request body or an `X-HTTP-Method-Override` header.
- If found and valid (`PUT`, `PATCH`, or `DELETE`), the request method is overridden.
- The `_method` field is removed from `req.body` so it doesn't leak into your controller logic.

### Configuration Options

```typescript
methodSpoofing({
  fieldName: "_method", // Form field name (default: "_method")
  checkHeader: true, // Also check X-HTTP-Method-Override header (default: true)
});
```

### Using in Templates

Use the `@method` directive inside a `POST` form:
...
# ORM / Driver selection: jcc (Knex), sequelize, or mongoose
DB_ORM=jcc
...
JCC Express MVC supports three database drivers through a unified `Database` module:

| Driver     | `DB_ORM`    | `DB_CONNECTION` | Package                          |
|------------|-------------|-----------------|----------------------------------|
| **Knex**   | `jcc`       | `mysql2`, `postgres`, `sqlite` | Built-in (Jcc-eloquent) |
| **Sequelize** | `sequelize` | `mysql2`, `postgres` | `npm install sequelize mysql2` |
| **Mongoose**  | `mongoose`  | `mongodb`       | `npm install mongoose`          |

**Knex (Jcc-eloquent):** The default ORM. Use Jcc-eloquent models, `DB.table()`, migrations, and the query builder. No extra packages for MySQL/PostgreSQL/SQLite.

**Sequelize:** For SQL databases with Sequelize models. Models in `app/Models` are auto-loaded and synced. Use `app.resolve("models").User` in controllers.

**Mongoose:** For MongoDB. See [MongoDB / Mongoose](#mongodb--mongoose) below.

#### MongoDB / Mongoose

To use MongoDB with Mongoose:
...
2. Set in `.env`:
   ...
   ```
   Or use individual variables (MongooseDriver will build the URI):
   ```env
   DB_HOST=127.0.0.1
   DB_PORT=27017
...
3. Generate a Mongoose model: `bun artisanNode make:model User` (when `DB_ORM=mongoose`)

**Mongoose model format** (created by `make:model` when `DB_ORM=mongoose`):
...
#### Sequelize Model Format (app/Models)

When using `DB_ORM=sequelize`, models in `app/Models` must export a factory function that receives `(sequelize, DataTypes)` and returns the model. Define `associate` for relationships. Parent models (e.g. User) should be alphabetically before child models (e.g. Post) so foreign keys resolve correctly:
...
```typescript
import { Database } from "jcc-express-mvc/lib/Database";

const db = new Database();
await db.connect();
const connection = db.getConnection(); // Knex | Sequelize | Mongoose

// With explicit driver:
const db = new Database({ driver: "sequelize" });
await db.connect();
```

#### Resolving Models in Controllers (Sequelize)

When using Sequelize, models are registered in the container and available as `models`:
...
export class UsersController {
  User: any;
  constructor() {
    this.User = app.resolve("models").User;
  }
...
The `DatabaseServiceProvider` syncs Sequelize models on boot using `sequelize.sync({ alter: true })`, which:

- Creates tables if they do not exist
- Alters existing tables to match model definitions (adds/removes columns)
- Does **not** drop tables or data (unlike `force: true`)

For development, you can set `DB_SYNC=true` or `APP_DEBUG=true` if you want different sync behavior (e.g. force recreate).

### Supported Databases

JCC Express MVC supports the following database systems:

- **MySQL** 5.7+ / MariaDB 10.3+ (via Knex or Sequelize)
- **PostgreSQL** 10.0+ (via Knex or Sequelize)
- **SQLite** 3.8.8+ (via Knex or Sequelize)
...
### Forms & Security Directives

#### CSRF Directive

The `@csrf` directive generates a hidden CSRF token input field. This is used to protect your forms against cross-site request forgery attacks. Requires the `csrf()` middleware to be active (see [CSRF Protection](#csrf-protection)).
...
HTML forms only support `GET` and `POST`. The `@method` directive generates a hidden input to spoof `PUT`, `PATCH`, or `DELETE` methods. Requires the `methodSpoofing()` middleware to be active (see [Method Spoofing](#method-spoofing)).
...
You can use `@method('PUT')`, `@method('PATCH')`, or `@method('DELETE')`.
...
JCC Express MVC supports three queue drivers: **memory**, **database**, and **redis**.

### Memory Driver

The memory driver stores jobs in a JSON file by default, so both the web server and `queue:work` share the same queue. No database or Redis needed.
...
Set `processSync: true` to process jobs immediately on push (no `queue:work` needed).

**Use memory driver when:**

- Getting started or developing locally
- You want queue:work to work without database/Redis

### Database Driver

The database driver stores jobs in a `jobs` table. Both the web server and queue worker share the same database, so jobs persist across processes.

**Use database driver when:**

- Running a separate queue worker (`bun artisanNode queue:work`)
- Deploying to production with background job processing
- You need jobs to survive process restarts

Set `QUEUE_CONNECTION=database` in your `.env` and run migrations (`bun artisanNode make:queue-table` then `bun artisanNode migrate`).

### Redis Driver

The Redis driver stores jobs in Redis lists and sorted sets. It is fast and shared across processes, ideal for production with high throughput.

**Use Redis driver when:**

- Running a separate queue worker
- You need fast, scalable job processing
- Redis is already available (caching, sessions, etc.)

Configuration in `app/Config/queue.ts` supports either a URL or an options object:

```typescript
// Option 1: URL string
connection: process.env.REDIS_URL || "redis://127.0.0.1:6379",

// Option 2: Options object (ioredis)
connection: {
  host: "127.0.0.1",
  port: 6379,
...
Set `QUEUE_CONNECTION=redis` in your `.env`. Use `REDIS_URL` for URL, or `REDIS_HOST`, `REDIS_PORT`, `REDIS_USERNAME`, `REDIS_PASSWORD`, `REDIS_DB` for the options format.

**Note:** The Redis driver requires the `ioredis` package. Install it only when using Redis: `npm install ioredis`. If you select the redis driver without it, you'll get a clear error with installation instructions.
...
export class ProcessPodcast extends Job<Podcast> {
  async handle() {
    // Access payload via this.data
...
### Job Path (Nested Folders)

You can organize jobs in subfolders within `app/Jobs`. When a job lives in a nested folder, set the `path` property so the queue worker can load it correctly.

**Default (no path):** Jobs in the root `app/Jobs` directory use the constructor name automatically.

```typescript
// app/Jobs/ProcessEmail.ts
import { Job } from "jcc-express-mvc/Core/Job";

export class ProcessEmail extends Job<any> {
  async handle() {
    // Resolves to app/Jobs/ProcessEmail
  }
}
```

**Custom path (nested folders):** Set `path` to the relative path within `app/Jobs`.

```typescript
// app/Jobs/Notifications/SendEmail.ts
import { Job } from "jcc-express-mvc/lib/Queue/Job";

export class SendEmail extends Job<any> {
  path = "Notifications/SendEmail"; // Required for nested folders

  async handle() {
    // Resolves to app/Jobs/Notifications/SendEmail
  }
}
```

- If `path` is empty or not set, the default is the constructor name (e.g. `ProcessEmail`).
- If `path` is set, it must match the file location (e.g. `Notifications/SendEmail` for `app/Jobs/Notifications/SendEmail.ts`).
- The path is used for dynamic loading when the worker processes jobs and for logging (e.g. `app/Jobs/Notifications/SendEmail`).

## Dispatching Jobs

Once you have written a job class, you may dispatch it in two ways:

**Option 1: Static `dispatch()` on the job class** (recommended)

```typescript
import { ProcessEmail } from "@/Jobs/ProcessEmail";

// Dispatch the job to the queue
await ProcessEmail.dispatch({ to: "user@example.com", subject: "Hello" });
```

If the job has `delay` set, it will automatically use `queue.later()`.

**Option 2: Via the Queue service**
...
To delay the execution of a queued job, set `job.delay` (in seconds) and dispatch, or use `queue.later()`:

```typescript
// Option 1: Set delay on the job, then dispatch (global helper or static dispatch)
const job = new ProcessPodcast(podcast);
job.delay = 5; // 5 seconds
await dispatch(job);

// Option 2: Use queue.later(job, delayInSeconds, queue?)
await queue.later(new ProcessPodcast(podcast), 5); // 5 seconds
```

### Pushing Multiple Jobs

You can push multiple jobs to the queue in sequence. Each `push` returns a `Promise<string>` (job id):

```typescript
await queue.push(new ProcessPodcast(podcast));
await queue.push(new OptimizePodcast(podcast));
await queue.push(new PublishPodcast(podcast));
```

## Running The Queue Worker

When using the **database** or **redis** driver, run a separate worker process to process queued jobs. The **memory** driver with `processSync: true` processes jobs immediately — no worker needed.
...
Sometimes you may wish to prioritize how your queues are processed. For example, in your `app/Config/queue.ts` file you may set the default queue for your connection. To push jobs to a specific queue, pass the queue name as the second argument to `push()` or `later()`:

```typescript
await queue.push(new ProcessPodcast(podcast), "high");
// Or for delayed jobs:
await queue.later(new ProcessPodcast(podcast), 5, "high");
```
...
// In your controller - pass data object matching Job<T> generic
await dispatch(new SendWelcomeEmail({ email: user.email, name: user.name }));
```
...
#### File Extension (FILE_EXT) and Model Template (DB_ORM)

**FILE_EXT** — The `make:controller`, `make:model`, `make:migration`, and other `make:*` commands generate either TypeScript (`.ts`) or JavaScript (`.js`) files based on `FILE_EXT` in your `.env`:
...
- `FILE_EXT=ts` — Generated files use `.ts` extension (default)
- `FILE_EXT=js` — Generated files use `.js` extension for JavaScript projects

Set this before running any `make:*` command. It affects controllers, models, migrations, seeders, jobs, events, listeners, policies, and commands.

**DB_ORM** — The `make:model` command scaffolds different model formats based on `DB_ORM`:
- `DB_ORM=jcc` → Jcc-eloquent Model class (extends Model)
- `DB_ORM=sequelize` → Sequelize factory function
- `DB_ORM=mongoose` → Mongoose schema and model
...
- Enable CSRF protection with `csrf()` middleware for form-based routes (see [CSRF Protection](#csrf-protection))
- Use `@csrf` and `@method` directives in all forms
